...
if ($GLOBALS['TL_CONFIG']['useRTE']):
    ...
    $res = $this->Database->prepare("select title, alias from tl_regina where alias != 'default'")
        ->execute();
    $result = $res->fetchAllAssoc();
    $res = array();
    foreach ($result as $reginaType) {
        $res[] = implode(',', $reginaType);
    }
    $reginaType = implode(';', $res);
    ?>
    <script src="<?php echo $this->base; ?>assets/tinymce/tiny_mce_gzip.js"></script>
    <script>
        tinyMCE_GZ.init({
            plugins: "advimageRegina,autosave,directionality,emotions,inlinepopups,paste,save,searchreplace,style,tabfocus,table,template,typolinks,xhtmlxtras",
            themes: "advanced",
    ...
    <script>
        tinyMCE.init({
            ...
            reginaTypes: "<?php echo $reginaType; ?>"
        });
    </script>
<?php endif; ?>
